...
void choleskyFactorization(const int NbLoops, SpBlas::Block blocks[], const int inMatrixDim, const int inBlockDim){
    ...
    struct __has_extension{
     ...
     std::vector<HipHandles> handles(ce.getNbhipWorkers());
     ...
     ce.execOnWorkers([&handles, offsetWorker, &ce, inBlockDim, &blocks](auto id, auto type){
         ...
         if(type == SpWorkerTypes::Type::HIP_WORKER){
             ...
             CUBLAS_ASSERT(cublasSetStream(hdl.blasHandle, SpHipUtils::GetCurrentStream()));
             CUSOLVER_ASSERT(cusolverDnCreate(&hdl.solverHandle));
             CUSOLVER_ASSERT(cusolverDnSetStream(hdl.solverHandle, SpHipUtils::GetCurrentStream()));
            ...
            for(int m = k + 1 ; m < nbBlocks ; ++m){
                ...
                if(m % Psize == Prank){
                  ...
                  , SpHip([inBlockDim, offsetWorker, &handles](const SpDeviceDataView<const SpBlas::Block> paramA,
                        ...
                        assert(idxHipWorker < int(handles.size()));
                        const double one = 1;
                        CUBLAS_ASSERT( cublasDtrsm( handles[idxHipWorker].blasHandle, CUBLAS_SIDE_RIGHT, CUBLAS_FILL_MODE_UPPER,
            ...
            for(int n = k+1 ; n < nbBlocks ; ++n){
                ...
                if(n % Psize == Prank){
                  ...
                  , SpHip([inBlockDim, offsetWorker, &handles](const SpDeviceDataView<const SpBlas::Block> paramA,
                        ...
                        CUBLAS_ASSERT( cublasDsyrk( handles[idxHipWorker].blasHandle, CUBLAS_FILL_MODE_UPPER, CUBLAS_OP_N,
                ...
                for(int m = k+1 ; m < nbBlocks ; ++m){
                    ...
                    if(n % Psize == Prank){
                      ...
                      , SpHip([inBlockDim, offsetWorker, &handles](const SpDeviceDataView<const SpBlas::Block> paramA,
                            ...
                            const double one = 1;
                            const double minusone = -1;
                            CUBLAS_ASSERT( cublasDgemm( handles[idxHipWorker].blasHandle, CUBLAS_OP_N, CUBLAS_OP_T,
