...
void gemm(const int NbLoops, SpBlas::Block blocksC[], const SpBlas::Block blocksA[], const SpBlas::Block blocksB[],
    ...
    const int nbBlocks = (inMatrixDim+inBlockDim-1)/inBlockDim;
     ...
     std::vector<cublasHandle_t> handles(runtime.getNbHipWorkers());
     ...
     runtime.execOnWorkers([&handles, offsetWorker, &runtime](auto id, auto type){
         ...
         if(type == SpWorkerTypes::Type::HIP_WORKER){
            ...
            CUBLAS_ASSERT(cublasSetStream(hdl, SpHipUtils::GetCurrentStream()));
         }
     });
#endif
    for(int idxLoop = 0 ; idxLoop < NbLoops ; ++idxLoop){
                ...
                for(int k = 0 ; k < nbBlocks ; ++k){
                    runtime.task(SpPriority(1), SpCommutativeWrite(blocksC[i*nbBlocks+j]),
                      ...
                      , SpHip([inBlockDim, offsetWorker, &handles](SpDeviceDataView<SpBlas::Block> paramC, const SpDeviceDataView<const SpBlas::Block> paramA,
                            ...
                            CUBLAS_ASSERT( cublasDgemm( handles[idxHipWorker], CUBLAS_OP_N, CUBLAS_OP_N,
