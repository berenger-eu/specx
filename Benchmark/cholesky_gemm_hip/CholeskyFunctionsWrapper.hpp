...
inline const char* cusolverGetStatusString(cusolverStatus_t status)
{
    ...
    case CUSOLVER_STATUS_NOT_INITIALIZED: return "CUSOLVER_STATUS_NOT_INITIALIZED The cuSolver library was not initialized. This is usually caused by the lack of a prior call, an error in the HIP Runtime API called by the cuSolver routine, or an error in the hardware setup. To correct: call cusolverCreate() prior to the function call; and check that the hardware, an appropriate version of the driver, and the cuSolver library are correctly installed.";
    case CUSOLVER_STATUS_ALLOC_FAILED : return "CUSOLVER_STATUS_ALLOC_FAILED Resource allocation failed inside the cuSolver library. This is usually caused by a hipMalloc() failure. To correct: prior to the function call, deallocate previously allocated memory as much as possible.";
...
{\
    cublasStatus_t ___resHip = (X);\
    if ( CUBLAS_STATUS_SUCCESS != ___resHip ){\
    printf("Error: fails, %s (%s line %d)\n", cublasGetStatusString(___resHip), __FILE__, __LINE__ );\
    exit(1);\
    }\
    }
...
{\
    cusolverStatus_t ___resHip = (X);\
    if ( CUSOLVER_STATUS_SUCCESS != ___resHip ){\
    printf("Error: fails, %s (%s line %d)\n", SpBlas::cusolverGetStatusString(___resHip), __FILE__, __LINE__ );\
    exit(1);\
    }\
    }
